...
/datum/nano_module/xenopanel/Topic(href, href_list, state)
	...
	if (href_list["mode"])
		...
		if(check_rights(R_SERVER) && length(notlist))
...
/datum/nano_module/xenopanel/proc/upload_SQL(client/user, list/grant, list/revoke)
	. = 1
...
/proc/GetXenoWhitelist(not_used_list = FALSE)
...
/proc/xeno_diff_list(list/a, list/b)
	var/list/result = list()
	for(var/check in a)
